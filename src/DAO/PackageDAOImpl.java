...
public class PackageDAOImpl implements PackageDAO {
	...
	@Override
	public int update_locationofPackage(String location, int Consign_id,String status) throws SQLException, ClassNotFoundException {
		int count=0;
		try {	
			System.out.println(location+"    "+ Consign_id);
		Connection con = ConnectionClass.getConnection();
		String qry="Update Package set Current_Location='"+location+"' ,Package_Status='"+status+"' where Consignment_Id="+Consign_id;
	...
	@Override
	public String validateId(int consign_id) {
		String resp=null;
		try {
			Connection con = ConnectionClass.getConnection();
			String qry="Select * from Package where Consignment_Id="+consign_id;
			Statement Stmt=con.createStatement();
			ResultSet rs=Stmt.executeQuery(qry);
			
			if(rs.next()) {
			resp=rs.getString(9)+"@"+rs.getString(10);	
			}
		}catch(Exception E) {
			System.out.println("Error at validating consignment Id : "+E);
		}
		return resp;
	}
